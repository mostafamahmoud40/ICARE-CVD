...
import { formatNotificationTime, getDoctorNotificationMeta } from "./doctorNotifications.utils"

const ICON_FIRST_KINDS = new Set<DoctorNotification["kind"]>([
  "medication_flag",
  "ai_insight",
  "system",
  "lab_result",
...
function usesIconPresentation(notification: DoctorNotification) {
  return !notification.sender || ICON_FIRST_KINDS.has(notification.kind)
}

function actionButtonClass(variant: DoctorNotificationAction["variant"] = "secondary", compact?: boolean) {
  return cn(
...
function PersonNotificationContent({
  ...
  notification: DoctorNotification & { sender: NonNullable<DoctorNotification["sender"]> }
  compact?: boolean
}) {
  return (
    <>
...
function SystemNotificationContent({
  ...
  const { icon: Icon, accent } = getDoctorNotificationMeta(notification.kind)
...
export function DoctorNotificationListItem({
  ...
  const rowClass = cn(
    ...
    hasActions ? "hover:bg-[#F9F8F5]" : "hover:bg-[#F9F8F5]",
  )
