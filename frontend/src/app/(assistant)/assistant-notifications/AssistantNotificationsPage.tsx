...
import {
  ASSISTANT_NOTIFICATION_KIND_OPTIONS,
  filterAssistantNotifications,
  type AssistantNotificationKindFilter,
} from "./assistantNotifications.filters"
import { useAssistantNotifications } from "./useAssistantNotifications"

export function AssistantNotificationsPage() {
  const [kindFilter, setKindFilter] = useState<AssistantNotificationKindFilter>("all")
  const [searchQuery, setSearchQuery] = useState("")
  const { notifications, unreadCount, markAllAsRead, resolveAction } = useAssistantNotifications()

  const filteredNotifications = useMemo(
    () => filterAssistantNotifications(notifications, kindFilter, searchQuery),
  ...
  return (
    <div className="flex w-full flex-1 flex-col space-y-6 p-6 sm:p-8">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <Select
          value={kindFilter}
          onValueChange={(value) => setKindFilter(value as AssistantNotificationKindFilter)}
        >
          ...
          <SelectContent>
            {ASSISTANT_NOTIFICATION_KIND_OPTIONS.map((option) => (
              ...
              <div key={notification.id} className="px-2 py-1">
                <AssistantNotificationListItem
                  ...
                  onAction={(notificationId, action) =>
                    handleAssistantNotificationAction(notificationId, action, resolveAction)
                  }
                />
              </div>
            ))}
