...
import cv2
import torch
from utils.data_utils import convert_masks, visualize
import pandas as pd
...
def get_images(img, input_size=(224,224,1)):
    ...
    img = np.asarray(img)
    if img.ndim == 4:
        # Flatten time dimension into slices for inference
        img = img.reshape(img.shape[0], img.shape[1], -1)
    elif img.ndim != 3:
        raise ValueError(f"Unsupported image shape {img.shape}, expected 3D or 4D")
...
def get_vae_bottleneck(ED_ES_seg, vae):
    ...
    with torch.no_grad():
        x = torch.from_numpy(ED_ES_seg.reshape(ED_ES_seg.shape[0]*2, 1, 224, 224)).to(device)
        X_train = vae.bottle_neck(x)
        X_train = X_train.reshape(ED_ES_seg.shape[0], 2*16).cpu().numpy()
    return X_train

def pipeline(ED_ES, seg_model, vae, scaler, hdr_ed_patient, hdr_es_patient, affine_ed, affine_es):
    ...
    return batch_X_f, patient, batch_seg



def _ensure_monotonic_attr(model):
    """Backfill missing monotonic_cst attribute for older pickled trees."""
    ...
    if hasattr(model, "estimators_"):
        for est in model.estimators_:
            _ensure_monotonic_attr(est)


def classify(img_ED, img_ES, seg_model, vae, classification_model, scaler, hdr_ed_patient, hdr_es_patient, affine_ed, affine_es):
    ...
    deep_manual_features, manual_features_df, batch_seg = pipeline(ED_ES, seg_model, vae, scaler, hdr_ed_patient, hdr_es_patient, affine_ed, affine_es)
    
    print("manual_features.shape", manual_features_df.shape)
    print("deep_manual_features.shape",deep_manual_features.shape)
    
    _ensure_monotonic_attr(classification_model)
    preds = classification_model.predict(deep_manual_features)
    print("preds,", preds)
    preds = list(preds)
    pred_class = most_frequent(preds)

    return categories[pred_class], manual_features_df, batch_seg, img_ED, img_ES
