...
# Training-only import; provide a stub when dataset module is absent (inference path)
try:
    from utils.vae.dataset import get_loader  # type: ignore
except Exception:  # pragma: no cover - inference fallback
    def get_loader(*args, **kwargs):
        raise ImportError("utils.vae.dataset is required only for VAE training; not needed for inference")
import torch
